@section('title', 'Auto Comment') @include('default/header')
<script>
    $(".Comment a").addClass("activeSidebar");
</script>
                ...
                <div class="d-flex flex-column w-xl b-r white modal-dialog">
                    <div class="scrollable hover">
                        <div class="list inset">
                            ...
                            <div class="list-item">
                                <a href="{{ url('auto-comment/'.$showaccounts->username) }}"><span class="w-40 avatar circle blue" style="background: linear-gradient(to right, #667eea, #764ba2)!important"><!-- <img src='{{ asset("$showaccounts->profilepic") }}' alt='.'> -->
                                    {{ mb_strtoupper(mb_substr($showaccounts->username, 0, 2)) }}
                                </span></a>
                                <div class="list-body"><a href="{{ url('auto-comment/'.$showaccounts->username) }}" class="item-title _500">{{ $showaccounts->username }}
            ...
            <div class="content-main d-flex flex zero" id="content-main">
                <div class="d-flex flex" data-plugin="chat">
                    <div class="col-sm">
                        ...
                        <form method="POST" action="{{ url('updatecomment') }}">
                            @csrf
                            <input type="hidden" name="username" id="username" value="{{ $username }}">
                            <div class="box">
                                <label class="form-control myLabel">Auto Comment for <b>{{ $username }}</b></label>
                                <div class="box-body">
                                    ...
                                    <div class="input-group">
                                        <div class="col-sm" id="spanInsertUsername">
                                            ...
                                            <span class="badge badge-dark">{{ $showtarget }} &nbsp; @php $target = str_replace("#", "$", $showtarget); @endphp <a href="{{ url('deletetarget/'.$username.'/comment/'.$target) }}"><i class="fas fa-times"></i></a></span> @endforeach @endif
                                    ...
                                    <div class="row">
                                        <div class="col-sm">
                                            <div class="inputGroup form">
                                                {!! str_replace("value='".$timelinefeed."'","value='".$timelinefeed."' checked","
                                                <input id='option1' name='timelinefeed' value='1' type='checkbox'/>") !!}
                                                <label for="option1">Comment to the feed from the timeline</label>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-sm">
                                            <div class='input-group mb-3'>
                                                <div class="select animated zoomIn">
                                                    <input type="radio" name="commentgroup" required="">
                                                    <i class="toggle icon icon-arrow-down"></i>
                                                    <i class="toggle icon icon-arrow-up"></i>
                                                    <span class="placeholder">Select a Comment Group</span>
                                                    @foreach($groups as $showgroups)
                                                    {!! str_replace('value="'.$commentgroup.'"','value="'.$commentgroup.'" checked','
                                                        <label class="option bot">
                                                            <input type="radio" name="commentgroup" value="'.$showgroups->groupname.'">
