    ...
    public function validate($data = array())
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($data['csrf_token'] != $_SESSION['csrf_token']) {
                ...
                $this->inputPost($data['code'],'Codecanyon Purchase Code',['required']);

                if (isset($data['isMailInfo']) && $data['isMailInfo'] ==1 )
                {
                    $this->inputPost($data['mail_driver'],'Mail Driver',['alphaNumericDigit','alphaNumericDigit']);
                    $this->inputPost($data['mail_host'],'Mail Hostname',['alphaNumericDigit','alphaNumericDigit']);
                    $this->inputPost($data['mail_port'],'Mail Port',['port']);
                    $this->inputPost($data['mail_username'],'Mail Username',['alphaNumericDigit']);
                    $this->inputPost($data['mail_password'],'Mail Password',['password']);   
                }

                if (!file_exists(SQL_FILE_PATH)) {
                    $this->exception[] = "Database is not uploaded!";
                }
